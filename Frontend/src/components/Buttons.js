...
export const Buttons = (props) => {
  const urlAdd = "http://localhost:8080/backend/AddData?"
  const urlEdit = "http://localhost:8080/backend/Edit?";
...
const onDeleteSubmit = async (e) => {
  ...
  Axios.get('http://localhost:8080/backend/Delete?' + resp)
    .then(res => {
      console.log(res.data)
...
const onSearchChange = async (e) => {
  ...
  try {
    const res = await fetch("http://localhost:8080/backend/Search?" + resp);
...
{/* -------------this is the code for Advance Search functionality-------------- */}
  

  const onAdvsearchChange = (e) => {
    
    setAdvsearchForm({ ...advsearchForm, [e.target.name]: e.target.value});
    
  };

  const onAdvsearchSubmit = async () => {
    var resp = "doc_id=" + advsearchForm.doc_id + 
    "&invoice_id=" + advsearchForm.invoice_id + 
    "&cust_number=" + advsearchForm.cust_number + 
    "&business_year=" + advsearchForm.business_year;

    //e.preventDefault();

    try {
      const res = await fetch("http://localhost:8080/backend/AdvSearch?" + resp);
      const data = await res.json();
      props.setTableData(data);
      console.log(data);
      handleSearchClose();
    ...
    console.log(advsearchForm, resp);
    handleSearchClose();
    props.setSearchButton(false);
     ...
     {/* -------------this is the code for Predict functionality-------------- */}
    ...
    <div className='buttons'>
      <Stack direction="row" spacing={2} sx={[{'css-1ja9oug-MuiStack-root':{justifyContent: "center"}}]}>
        <ButtonGroup aria-label="outlined primary button group">
          <Button onClick={submitPredict} variant="outlined" size="large" sx={{color: "white", height: "40px", backgroundColor: "#14aff1"}}>Predict</Button>
            ...
            <DialogTitle style={{ color: "white", backgroundColor: "#2d4250" }}>Advance Search</DialogTitle>
              ...
              {/* <Box sx={{ width: '100%' }}> */}
                  <Stack direction="row" spacing={3}>
                    <Item><TextField onChange={onAdvsearchChange} label="Document ID" name="doc_id" placeholder="Document ID" sx={formStyle} autoFocus /></Item>
                    <Item><TextField onChange={onAdvsearchChange} label="Invoice ID" name="invoice_id" placeholder="Invoice ID" sx={formStyle} /></Item>
                  </Stack>
                  <Stack direction="row" spacing={3}>
                    <Item><TextField onChange={onAdvsearchChange} label="Customer Number" name="cust_number" placeholder="Customer Number" sx={formStyle} autoFocus /></Item>
                    <Item><TextField onChange={onAdvsearchChange} label="Business Year" name="business_year" placeholder="Business Year" sx={formStyle} /></Item>
                  </Stack>
              {/* </Box> */}
              </FormControl>
            </DialogContent>
            <DialogActions style={{ backgroundColor: "#2d4250" }}>
            <Button variant="outlined" size="large" onClick={onAdvsearchSubmit} style={{ width: "50%" }}> Search </Button>
        ...
        <FormControl sx={[{ width: '25ch', padding: "2px 25px 7px" }]}>
          <OutlinedInput onChange={onSearchChange} type="text" placeholder="Search Customer ID" sx={searchformStyle} />
